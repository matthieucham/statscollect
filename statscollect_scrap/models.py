...
class ScrapedDataSheet(models.Model):
    ...
    def __str__(self):
        return "[%s] %s %s - %s %s (%s)" % (
            self.source, self.content['home_team'], self.content['home_score'] if 'home_score' in content else '?',
            self.content['away_score'] if 'away_score' in content else '?', self.content['away_team'],
