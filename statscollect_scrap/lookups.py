...
#     def get_query(self, request, term):
#         return (
#             super(GamesheetLookup, self)
#             .get_query(request, term)
#             .filter(source__in=["WHOSC", "LFP"])
#             .order_by("-match_date")
#         )
...
class RatingsheetLookup(ModelLookup):
    ...
    def get_query(self, request, term):
        ...
        if gs and instance:
            ...
            sheet_choices = dict(
                [
                    (
                        elem["hash_url"],
                        "%s=%s"
                        % (elem["content"]["home_team"], elem["content"]["away_team"]),
                    )
                    for elem in models.ScrapedDataSheet.objects.filter(
                        Q(source__in=("HDM",))
                        | Q(
                            match_date__range=(
                                datetime.datetime.combine(
                                    gamesheet.match_date.date(), datetime.time.min
                                ),
                                datetime.datetime.combine(
                                    gamesheet.match_date.date(), datetime.time.max
                                ),
                            ),
                        )
                    )
            ...
            preserved = Case(
                *[When(pk=pk, then=pos) for pos, pk in enumerate(found_ids)]
            )
            return (
                super(RatingsheetLookup, self)
                .get_query(request, term)
                .filter(hash_url__in=found_ids)
                .order_by(preserved)
            )
        return list([])
...
# @register("gamesheet")
# class GamesheetLookup(LookupChannel):
#     model = models.ScrapedDataSheet
...
# @register("ratingsheet")
# class RatingsheetLookup(LookupChannel):
#     model = models.ScrapedDataSheet
#     min_length = 3
